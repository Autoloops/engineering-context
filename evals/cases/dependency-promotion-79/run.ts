...
async function main(): Promise<void> {
  ...
  const targetCommit = prepareTargetRepo(context);
...
function prepareRun(): RunContext {
  ...
  const fixtureDir = resolve(repoRoot, "evals/cases/dependency-promotion-79/fixture-repo");
  ...
  const rubricPath = resolve(repoRoot, "evals/cases/dependency-promotion-79/rubric.json");
...
function prepareTargetRepo(context: RunContext): string {
  cpSync(context.fixtureDir, context.targetRepoDir, { recursive: true });
...
function writeResult(
  ...
  const result: EvalResult = {
    case_id: caseId,
    target_repo_url: `local-fixture:${context.fixtureDir}`,
...
function parseArgs(args: string[]): Args {
  ...
  if ((proposal === undefined && agent === undefined) || (proposal !== undefined && agent !== undefined)) {
    throw new Error("Usage: npm run eval:dependency-promotion-79 -- (--proposal /path/to/proposal.json | --agent codex) [--agent-model model] [--judge openai] [--judge-model model]");
  }
  ...
  const response = await fetch("https://api.openai.com/v1/responses", {
    ...
    body: JSON.stringify({
      ...
      text: {
        format: {
          type: "json_schema",
          name: "dependency_promotion_eval_judge",
          strict: true,
