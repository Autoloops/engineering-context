...
greplica install --platform <codex|claude|copilot|continue|opencode|openhands|factory-droid> --embedding local <mapped-hook-and-memory-flags>
```
...
```bash
greplica transcript bundle --platform <codex-or-claude-or-copilot-or-continue> --file <path-1> [--file <path-2>] [--file <path-3>] --out <greplica-transcript-backfill.md>
```
