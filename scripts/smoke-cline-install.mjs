...
function checkRuleInstall() {
  ...
  for (const pattern of ["Installed Greplica for Cline", "Skills:", "Hooks: installed for beforeModel", "Automatic memory updates: disabled."]) {
    if (!first.stdout.includes(pattern)) details.push(`install output missing ${JSON.stringify(pattern)}`);
  }
  if (first.stdout.includes("Guidance:")) details.push("install output should not include platform-specific guidance file reporting");
  if (readConfig().session.autoMemoryUpdates !== false) details.push("default Cline install should disable automatic memory updates");
  if (!second.stdout.includes("Hooks: installed for beforeModel")) details.push("reinstall did not report Cline plugin hook");
...
function checkHooksDisabled() {
  ...
  if (!existsSync(rulePath)) details.push("expected .clinerules/greplica.md to be created with hooks disabled");
    ...
    const rule = readFileSync(rulePath, "utf8");
    if (!rule.includes("greplica graph context")) details.push("hooks-disabled rule is missing shared Greplica guidance");
  }
